...
Using the reference azimuth convention, represent each source by the ordered
pair $(\theta,A)$, where $\theta$ is the source azimuth and $A$ is its relative
optical-intensity weight:

$$
\mathcal{S}_1=\lbrace(0^\circ,1),(90^\circ,2)\rbrace.
$$

$$
\mathcal{S}_2=\lbrace(45^\circ,1),(135^\circ,2)\rbrace.
$$
...
$$
(1,2)\longrightarrow\left(\frac13,\frac23\right).
$$
...
$$
\mathcal{E}=\lbrace4,5,6,11,12,13,14,15\rbrace.
$$
...
$$
\mathcal{O}=\lbrace7,8,9,10\rbrace.
$$
...
$$
\Omega_e=\Omega_{\mathrm{geom},e}\cap\Omega_{\mathrm{amp},e}\cap\Omega_{\mathrm{sens},e}.
$$
...
$$
\Omega_{m,n,s}=\Omega^{\mathrm{geom}}_{m,n,s}\cap\Omega_{\mathrm{amp}}^{Q}\cap\Omega_{\mathrm{amp},m}^{\mathrm{OAI}}\cap\Omega_{\mathrm{sens},m,n,s}^{o}.
$$

For the calibrated binary pupil support $A_p$, define the shifted-pupil branch indicators

$$
\chi_{m,n,s}(\mathbf{u})=A_p(\mathbf{u}_{m,n})A_p(\mathbf{u}_{m,n}+s\mathbf{u}),\qquad s\in\lbrace+1,-1\rbrace,
$$

and the branch count

$$
N_m(\mathbf{u})=\sum_{q\in\mathcal{S}_m}\sum_{s'\in\lbrace+1,-1\rbrace}\chi_{m,q,s'}(\mathbf{u}).
$$
...
$$
\widehat\gamma(\mathbf{u})=\frac{\mathrm{Re}(S_+^{\mathrm{filt}}(\mathbf{u})S_-^{\mathrm{filt}*}(\mathbf{u}))}{|S_-^{\mathrm{filt}}(\mathbf{u})|^2+\epsilon_\gamma}.
$$
...
$$
\widehat Q=-\frac{a_+^*S_+ + a_-^*S_-}{2(|a_+|^2+|a_-|^2)+\epsilon_Q}.
$$
...
$$
\widehat{\mathbf{c}}=\mathbf{A}^\dagger\mathbf{b}.
$$
...
$$
\widehat\phi(\mathbf{u})=\frac{H_{\mathrm{def}}^* I_{\mathrm{def}}+\sum_m H_{\mathrm{tilt},m}^*I_{\mathrm{tilt},m}}{|H_{\mathrm{def}}|^2+\sum_m|H_{\mathrm{tilt},m}|^2+\alpha(\mathbf{u})}.
$$

A reference energy-scaled choice is

$$
\alpha=10^{-12}\max_{\mathbf{u}}\left(|H_{\mathrm{def}}|^2+\sum_m|H_{\mathrm{tilt},m}|^2\right),
$$
...
- For a nonbinary least-squares weight $w(\mathbf{u})$, apply its square root $F(\mathbf{u})=\sqrt{w(\mathbf{u})}$ to both the measured spectrum and the corresponding transfer function; a binary mask is the special case $F\in\lbrace0,1\rbrace$.
