...
For pattern $m\in\lbrace 1,2\rbrace$, let $\mathcal S_m$ contain two mutually incoherent point sources. Each pair has a $90^\circ$ angular separation and calibrated intensity weights $A_{m,1}:A_{m,2}=1:2$.

A convenient azimuth convention is

$$
\mathcal S_1=\lbrace 0^\circ,90^\circ\rbrace,\qquad \mathcal S_2=\lbrace 45^\circ,135^\circ\rbrace.
$$

The corresponding transverse illumination wavevectors are
$\mathbf u_{m,n}$, where $n\in\lbrace 1,2\rbrace$.
...
$$
\mathcal E=\lbrace 4,5,6,11,12,13,14,15\rbrace.
$$

Odd-mode index set:

$$
\mathcal O=\lbrace 7,8,9,10\rbrace.
$$
...
- $\mathcal F\lbrace\cdot\rbrace$ and $\mathcal F^{-1}\lbrace\cdot\rbrace$: centered two-dimensional Fourier transform and inverse transform;
- $(\cdot)^*$: complex conjugate;
- $\mathrm{Arg}(\cdot)$: principal complex phase in $(-\pi,\pi]$;
...
- $s\in\lbrace +1,-1\rbrace$: positive or negative shifted-pupil branch.

The exact nonparaxial defocus phase is referenced to the pupil origin. In the notation of the manuscript,

$$
W_{\Delta z}(\mathbf u)=k\Delta z[\sqrt{1-\lambda^2\lVert\mathbf u\rVert^2}-1],
$$
