...
$$
\mathcal{S}_1=\bigl\{(0^\circ,1),\,(90^\circ,2)\bigr\}.
$$

$$
\mathcal{S}_2=\bigl\{(45^\circ,1),\,(135^\circ,2)\bigr\}.
$$
...
$$
\chi_{m,n,s}(\mathbf{u})=A_p(\mathbf{u}_{m,n})A_p(\mathbf{u}_{m,n}+s\mathbf{u}),\qquad s\in\{+1,-1\},
$$

and the branch count

$$
N_m(\mathbf{u})=\sum_{q\in\mathcal{S}_m}\sum_{s'\in\{+1,-1\}}\chi_{m,q,s'}(\mathbf{u}).
$$
...
$$
\widehat\gamma(\mathbf{u})=\operatorname{Re}\left\{\frac{S_+^{\mathrm{filt}}(\mathbf{u})S_-^{\mathrm{filt}*}(\mathbf{u})}{|S_-^{\mathrm{filt}}(\mathbf{u})|^2+\epsilon_\gamma}\right\}.
$$
